...
# JAVA应用程序的名称
APP_NAME=my-blog-layui-1.0.0-SNAPSHOT

# jar包存放路径
JAR_PATH='/home/filing/stats2/server'
# jar包名称
...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    # jar服务启动脚本
    nohup java $JAVA_OPTS -Xloggc:$LOG_FILE/gc/gclog.log -XX:HeapDumpPath=$LOG_FILE/gc/HeapDump.hprof  -jar $JAR_PATH/$JAR_NAME --spring.profiles.active=prod >./$LOG_FILE/run.log 2>&1 &
    echo $! > $JAR_PID
